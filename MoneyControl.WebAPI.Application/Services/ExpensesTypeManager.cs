...
namespace MoneyControl.WebAPI.Application.Services
{
    public class ExpensesTypeManager : IExpensesTypeManager
    {
        ...
        public async Task<ExpensesType> CreateNewExpensesType(ExpensesType expensesType, CancellationToken token)
        {
            var result = await _expensesTypeRepository.CreateAsync(expensesType, token);
            return result;
        }

        public async Task<ExpensesType> GetExpensesType(string Id, CancellationToken token)
        {
            return await _expensesTypeRepository.GetAsync(Id, token);
        }

        public async Task RemoveExpensesType(string Id, CancellationToken token)
        {
